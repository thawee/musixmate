package apincer.android.provider;
...
public class StorageProvider extends DocumentsProvider {
    ...
    private void updateVolumesLocked() {
        ...
        for (apincer.android.storage.StorageVolume storageVolume : storageUtils.getStorageMounts()) {
    ...
    public String getDocIdForFileMaybeCreate(File file, boolean createNewDir)
    ...
    public RootInfo getRootInfo(String path) {
        for(RootInfo root: mRoots.values()) {
            if(path.startsWith(root.path.getAbsolutePath())) {
                return root;
            }
        }
        return null;
    ...
    private void includeFile(MatrixCursor result, String docId, File file)
        ...
        if (Utils.mimeMatches(Utils.VISUAL_MIMES, mimeType)) {
