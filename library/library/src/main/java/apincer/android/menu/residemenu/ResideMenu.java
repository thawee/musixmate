package apincer.android.menu.residemenu;
...
public class ResideMenu extends FrameLayout {
    ...
    private void initViews(Context context, int customLeftMenuId,
        ...
        headerHolder = (RelativeLayout) findViewById(R.id.sv_header_holder);
        headerHolder.setVisibility(GONE);
    }
    ...
    private void addMenuItem(final ResideMenuItem menuItem, int direction) {
        if (direction == DIRECTION_LEFT) {
            menuItem.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    onMenuItemClickListener.onClick(menuItem.getMenuItem());
                }
            });
            this.leftMenuItems.add(menuItem);
            layoutLeftMenu.addView(menuItem);
    ...
    public void setCustomHeader(View customView) {
        headerHolder.removeAllViews();
        headerHolder.addView(customView);
        headerHolder.setVisibility(VISIBLE);
    }
    ...
    @Deprecated
    public void setMenuResRight(int menuRes) {
        MenuInflater inflater = new MenuInflater(getContext());
        inflater.inflate(menuRes, menuRight);
        for(int i=0; i<menuRight.size();i++) {
            final MenuItem item = menuRight.getItem(i);
            ResideMenuItem rItem = new ResideMenuItem(getContext(), item);
            addMenuItem(rItem, DIRECTION_RIGHT);
        }
    }

    @Deprecated
    public void setMenuResLeft(int menuRes) {
        MenuInflater inflater = new MenuInflater(getContext());
        inflater.inflate(menuRes, menuLeft);
        for(int i=0; i<menuLeft.size();i++) {
            final MenuItem item = menuLeft.getItem(i);
            ResideMenuItem rItem = new ResideMenuItem(getContext(), item);
            addMenuItem(rItem, DIRECTION_LEFT);
        }
    }
